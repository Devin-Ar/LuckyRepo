...
const ContinueView: React.FC = () => {
    ...
    const handleContinue = () => {
        CampaignManager.getInstance().retryCurrentStep();
    };
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', gap: '40px' }}>
                <button
                    ...
                    style={{ padding: '15px 30px', fontSize: '1.5rem', background: 'transparent', border: '3px solid white', color: 'white', cursor: 'pointer' }}
                >
                    YES (ENTER)
                </button>
                <button
                    onClick={handleGiveUp}
                    style={{ padding: '15px 30px', fontSize: '1.5rem', background: 'transparent', border: '3px solid #666', color: '#666', cursor: 'pointer' }}
                >
                    NO (ESC)
                </button>
            </div>
