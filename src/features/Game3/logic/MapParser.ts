...
export class MapParser {
    ...
    public static async parseMap(imagePath: string, mapScale: number = 1): Promise<ParsedMapData> {
        ...
        try {
                        ...
                        if (currentPlat &&
                            ...
                            currentPlat = {
                                ...
                                assetKey: isWall ? 'Platform Length' :
                                    (isFloor ? 'Platform Floor' :
                                        (isExit ? 'Exit Door' :
                                            (isFallthrough ? 'Platform Floor' : 'Platform Length')))
                            };
            ...
            console.log(`[MapParser] Parsing complete. Platforms found:
             ${platforms.length}, after merge: ${mergedPlatforms.length}`);
